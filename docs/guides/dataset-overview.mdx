...
<Note>
- If the tokenizer has a usable chat template, `make_squad_dataset` uses it automatically and masks non-answer tokens. Otherwise it uses prompt-and-answer formatting.
...
- **ColumnMappedTextInstructionDataset (generic instruction SFT)**
  ...
  - Notes:
    - Set `use_hf_chat_template: true` to opt into a usable tokenizer chat template. With `answer_only_loss_mask: true`, the dataset masks non-answer tokens; without a usable template it falls back to prompt-and-answer formatting.
    - Map-style, non-streaming dataset (supports `len(ds)` and `ds[i]`)
    - For streaming (including Delta Lake / Databricks), use `ColumnMappedTextInstructionIterableDataset` (see [Streaming Datasets](#streaming-datasets)).
  - Example YAML:
```yaml
dataset:
...
```yaml
packed_sequence:
  packed_sequence_size: 8192   # > 0 enables packing; 0 disables it
  packing_strategy: thd        # Optional; defaults to THD packing
```
The fine-tuning recipe passes these settings to its packed-dataset path. For THD packing, it disables packing when the model does not support `seq_lens`.
...
Built-in dataset makers return a Python list or a Hugging Face `Dataset`; each yielded sample includes a `conversation` field:
...
RDR and CORD-V2 return lists. MedPix and CommonVoice 17 return Hugging Face `Dataset` objects with lazy transforms. Each sample follows the conversation schema expected by `apply_chat_template`, e.g.:
```python
{
  "conversation": [
    {
...
```
Answer-only loss masking is built into the supported VLM collators. They derive assistant regions from the processor's chat template with `build_labels_from_template`; `start_of_response_token` is not a collate argument.
...
- **Chat templates**: SQuAD uses a tokenizer chat template automatically when available. Column-mapped datasets opt in with `use_hf_chat_template: true`; `ChatDataset` derives assistant masking from the tokenizer's chat template. VLM collators derive assistant labels from the processor template and do not accept `start_of_response_token`.
